...
function coqbot_update_comment() {
    ...
    if [ ! -z "${coq_pr_number}" ]; then
        ...
        comment_text="${comment_text}${nl}${nl}"'```'"${nl}$(git log -n 1 "${old_coq_commit}")${nl}"'```'
        comment_text="${comment_text}${nl}${nl}<details><summary>New Coq version ${new_coq_commit}</summary>"
        comment_text="${comment_text}${nl}${nl}"'```'"${nl}$(git log -n 1 "${new_coq_commit}")${nl}"'```'
        comment_text="${comment_text}${nl}${nl}[Diff: "'`'"${old_coq_commit}..${new_coq_commit}"'`'"](https://github.com/coq/coq/compare/${old_coq_commit}..${new_coq_commit})"
