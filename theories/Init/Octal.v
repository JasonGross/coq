...
(** These numbers coded in base 8 will be used for parsing and printing
    other Coq numeral datatypes in an human-readable way.
    See the [Number Notation] command.
    We represent numbers in base 8 as lists of octal digits,
...
(** Unsigned integers are just lists of digits.
    For instance, eight is (D1 (D0 Nil)) *)
...
(** For decimal numbers, we use two constructors [Octal] and
    [OctalExp], depending on whether or not they are given with an
    exponent (e.g., 0x1.a2p+01). [i] is the integral part while [f] is
    the fractional part (beware that leading zeroes do matter). *)

Variant octal :=
 | Octal (i:int) (f:uint)
 | OctalExp (i:int) (f:uint) (e:Decimal.int).

Scheme Equality for uint.
Scheme Equality for int.
Scheme Equality for octal.

Declare Scope oct_uint_scope.
Delimit Scope oct_uint_scope with ouint.
Bind Scope oct_uint_scope with uint.

Declare Scope oct_int_scope.
Delimit Scope oct_int_scope with oint.
Bind Scope oct_int_scope with int.

Register uint as num.octal_uint.type.
Register int as num.octal_int.type.
Register octal as num.octal.type.
...
Fixpoint double d :=
  ...
  | D2 d => D4 (double d)
  | D3 d => D6 (double d)
  | D4 d => D0 (succ_double d)
  | D5 d => D2 (succ_double d)
  | D6 d => D4 (succ_double d)
  | D7 d => D6 (succ_double d)
  end
...
with succ_double d :=
  ...
  | D2 d => D5 (double d)
  | D3 d => D7 (double d)
  | D4 d => D1 (succ_double d)
  | D5 d => D3 (succ_double d)
  | D6 d => D5 (succ_double d)
  | D7 d => D7 (succ_double d)
  end.
